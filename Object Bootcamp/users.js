...
function filterByCountry(users, countryCode) {
	const result = users.filter(user => user.nat === countryCode)
	return result
}

result = filterByCountry(users, countryCode)
...
function filterByGender(users, gender) {
	...
	const female = ['Miss', 'Ms', 'Mademoiselle', 'Mrs']

	let result;

	if (gender === 'male') {
		result = users.filter(user => male.includes(user.name.title))
	}
	else {
		result = users.filter(user => female.includes(user.name.title))
	}

	return result;
}

result2 = filterByGender(users, 'female')

//listEmails

function filterByEmail(users) {
	const result = users.filter(user => user.email)
	return result
}

result3 = filterByEmail(users)

let emailList = users.map(function(property) {
...
let newEmailList = users.map(function(property) {
	return `${property.name.last}.${property.name.first}@evilcorp.${property.nat}`
})
